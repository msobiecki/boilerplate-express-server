...
const { numberOfInstances } = env.cluster;

const numberOfWorkers = numberOfInstances || numberOfCPUs;

if (cluster.isPrimary) {
  logger.info(`Primary cluster process is running. Preparing ${numberOfWorkers} ${numberOfWorkers > 1 ? 'workers' : 'worker'}...`);
  ...
  cluster.on('exit', (worker, code, signal) => {
    ...
    if (code && code !== 0) {
      logger.error(`Worker ${pid} exited with error code ${code}. Restarting...`);
      cluster.fork();
    } else {
